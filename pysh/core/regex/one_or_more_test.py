...
from pysh.core.regex import Regex, Result, State, OneOrMore


def test_str():
    assert str(Regex.literal("a").one_or_more()) == "a+"
...
def test_apply(subtests):
    for input, expected in list[tuple[str, Optional[tuple[State, Result]]]](
        ...
        with subtests.test(input=input, expected=expected):
            regex = Regex.literal("a").one_or_more()
            if expected is None:
                pytest.raises(OneOrMore.Error, lambda: regex(input))
            else:
